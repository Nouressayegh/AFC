...
D = pd.DataFrame([D["Sexe"],D[" temps travail"]])
D = D.transpose()
k = D.shape[0]
I = max(D["Sexe"])+1
...
for i in range(1,k): 
   mat_contingence[D.loc[i,"Sexe"] , D.loc[i," temps travail"]] = mat_contingence[D.loc[i,"Sexe"] ,D.loc[i," temps travail"]] + 1

mat_contingence = pd.DataFrame(mat_contingence)
mat_contingence = pd.DataFrame([[4,40,41],[2,18,18],[0,2,1],[9,55,31],[9,7,10]])
mat_contingence.columns = ["Non répondu","Homme","Femme"]
...
mat_sommes["Somme"] = [np.sum(mat_sommes.loc["Non répondu",:]),np.sum(mat_sommes.loc["Homme",:]),np.sum(mat_sommes.loc["Femme",:])]
